...
def addEthicalData(request):
    if request.method == 'POST':
        ...
        data = {'user': inputData["user"], 'module_id': inputData["module_id"], 'scene': inputData["scene"], 'behavior_id' : inputData["behavior_id"], 'emotion': inputData["emotion"]}
...
def reportUsage(request):
    ...
    #print(usr)
    #player = Player.objects.get(user=usr)
    data = {'username': request.GET['email'], 'device_model': request.GET['model'], 'sisu_user' : sisuUsr, 'module_id' : request.GET['module_id'], 'ip_address' : 0}
